...
Triangle TriangleMaths::indexerToTri(TriIndexer idx, std::vector<OfxPointD> verticies) {
    return {verticies[idx.i1], verticies[idx.i2], verticies[idx.i3]};
}

BarycentricWeights TriangleMaths::toBarycentric(OfxPointD p, Triangle tri) {
...
OfxPointD TriangleMaths::fromBarycentric(BarycentricWeights bw, Triangle tri) {
    return {
...
bool TriangleMaths::circumCircleContains(Triangle tri, OfxPointD p) {
...
std::vector<TriIndexer> TriangleMaths::delaunay(std::vector<OfxPointD> &vertices)
{
